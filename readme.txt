...
2、常用命令
   ...
   --git commit xxx -m 'xxx' //把文件提交到仓库（8.PNG），引号里的是                               //注释，忘加这参数会进入编辑器，编辑、退                              //出方法 和（1）中一样，参数 -a 全部提交
...
4、以推送到github为例
   ...
   --之后本地仓库改动后只需输入 git push 即可修改远程仓库（18.PNG）

5、其他命令感觉对你不是很常用，也可以看一下，这网站挺全的
